	...
	return value
		...
		.map((line) => {
			...
			if (endedAt && Number.isNaN(endedAt.getTime())) {
				throw new Error(
					`Invalid ended_at value in time sessions: ${endedAtRaw}`
				);
			}
				...
				<section
					...
					<Card>
						<CardHeader className='border-b border-border'>
							...
							<CardDescription>
								Desktop-first task workspace. Search and filters are visible
								shell controls in this issue.
							</CardDescription>
						</CardHeader>
					...
					<Card className='min-h-[420px]'>
						<CardHeader className='border-b border-border'>
							...
							<CardDescription>
								Select a task row title to open the task detail view.
								Search/filter behavior will be wired in later issues.
							</CardDescription>
						</CardHeader>
						<CardContent>
							{tasks.length === 0 ?
								<Empty className='border border-dashed border-border bg-muted/20'>
									<EmptyHeader>
										...
										<EmptyDescription>
											Create your first task from the Quick Add panel on the
											right to start your list.
										</EmptyDescription>
									</EmptyHeader>
									<EmptyContent>
										<Button
						...
						<CardContent>
							{selectedTask ?
								<form
									...
									<div className='space-y-1.5'>
										<label
											htmlFor='detailTimeSessions'
											className='text-sm font-medium'>
											Time sessions (one line:
											startedAt|endedAt|durationSeconds)
										</label>
										<Textarea
											id='detailTimeSessions'
											name='detailTimeSessions'
											defaultValue={serializeTimeSessions(
												selectedTask.timeSessions
											)}
